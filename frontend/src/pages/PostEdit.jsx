  ...
  const PostEdit = () => {
    ...
    const loadPost = async () => {
      try {
        ...
        if (result.post_type === "photo" || result.post_type === "document") {
          const storedBust = localStorage.getItem(`post_bust_${result.id}`);
          setPreview(getPostFileUrl(result.id, storedBust));
    ...
    const restoreExistingFile = () => {
      ...
      if (post.post_type === "photo" || post.post_type === "document") {
        const storedBust = localStorage.getItem(`post_bust_${post.id}`);
        setPreview(getPostFileUrl(post.id, storedBust));
      }
    };
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        // stocker le timestamp dans le state
