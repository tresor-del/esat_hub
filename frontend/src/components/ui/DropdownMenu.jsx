...
const DropdownMenu = ({
  ...
  /**
* INTERCEPTION DES CLICS SUR LES ENFANTS
* Clone chaque élément enfant pour exécuter close() en plus de son onClick d'origine
*/
  const renderChildren = () => {
    return React.Children.map(children, (child) => {
      // Sécurité si l'enfant est null, undefined ou du texte brut
      if (!React.isValidElement(child)) return child;

      return React.cloneElement(child, {
        onClick: (e) => {
          // Exécute d'abord le onClick d'origine de l'élément s'il existe
          if (child.props.onClick) {
            child.props.onClick(e);
          }
          // Ferme le dropdown immédiatement
          close();
        },
      });
    });
